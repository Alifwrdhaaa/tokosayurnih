...
?> 	 
		...
		<?php 
			if(!empty($_POST['form-order'])){
				...
				$kt = mysqli_fetch_object(mysqli_query($conn,"SELECT *FROM kota where id='$kota'"));
				if($difference->days >= 0){
					
					$q = mysqli_query($conn,"insert into pesanan Values(NULL,'$tanggal_pesan','$tanggal_digunakan','$_SESSION[iam_user]','$nama','$alamat','$kt->nama','$kt->ongkir','$telephone','0','belum lunas')");
					if($q){ 
					
						$last = mysqli_fetch_array(mysqli_query($conn,"SELECT *FROM pesanan order by id DESC limit 1"));
						...
						foreach($cart as $id => $qty){
							...
							if(!empty($product)){ 
								...
								$ins = mysqli_query($conn,"insert into detail_pesanan Values(NULL,'$product[id]','$qty','$last[id]')");
					...
					?>
					<div class="alert alert-danger">Tanggal Penggunakan terlalu mepet. Pemesanan minimal 3 Hari sebelum hari H.</div>
					<?php
				}
			}
		?>
				 ...
				 <form action="" method="post" enctype="multipart/form-data">
						...
						<div class="form-group">
							<div class='input-group date' id='datetimepicker'>
								<input type='date' class="form-control" name="tanggal_digunakan" 
