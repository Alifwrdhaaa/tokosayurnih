<?php 
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			$q = mysqli_query($conn,"delete from kontak WHERE id='$_GET[id]'");
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($conn,"insert into kontak Values(NULL,'$nama','$email','$subjek','$pesan')");
			if($q){ alert("Success"); redir("kontak.php"); }
		}
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($conn,"update kontak SET nama='$nama',email='$email',subjek='$subjek',pesan='$pesan' where id=$_GET[id]") or die(mysql_error());
			if($q){ alert("Success"); redir("kontak.php"); }
		}
	}
...
?> 
	...
	<div class="container">
		<?php
			$q = mysqli_query($conn,"select * from kontak");
			$j = mysqli_num_rows($q);
		?>
		<h4>Daftar Kontak Masuk (<?php echo ($j>0)?$j:0; ?>)</h4>
		<a class="btn btn-sm btn-primary" href="kontak.php?act=create">Tambah Data</a>
		<hr>
		<?php
			if(!empty($_GET)){
				if($_GET['act'] == 'create'){
		...
		<?php while($data=mysqli_fetch_object($q)){ ?> 
				<tr> 
					...
					<td>
						...
						<a class="btn btn-sm btn-danger" href="kontak.php?act=delete&&id=<?php echo $data->id ?>">Hapus</a>
					</td> 
				</tr>
		<?php } ?>
			</tbody> 
