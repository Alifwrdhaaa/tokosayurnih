<?php 
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			$q = mysqli_query($conn,"delete from user WHERE id='$_GET[id]'");
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($conn,"insert into user Values(NULL,'$nama','$email','$telephone','$alamat','$password','$status')");
			if($q){ alert("Success"); redir("user.php"); }
		}
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			extract($_POST); 
			$password = md5($password);
			$q = mysqli_query($conn,"update user SET nama='$nama',email='$email',telephone='$telephone',alamat='$alamat',password='$password',status='$status' where id=$_GET[id]") or die(mysqli_error());
			if($q){ alert("Success"); redir("user.php"); }
		}
	}
...
?> 
	...
	<div class="container">
		<?php
			$q = mysqli_query($conn,"select*from user where status !='admin'");
			$j = mysqli_num_rows($q);
		?>
		<h4>Daftar user Masuk (<?php echo ($j>0)?$j:0; ?>)</h4>
		<a class="btn btn-sm btn-primary" href="user.php?act=create">Tambah Data</a>
		<hr>
		<?php
			if(!empty($_GET)){
				if($_GET['act'] == 'create'){
				...
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object(mysqli_query($conn,"select * from user where id='$_GET[id]'"));
				?>
					<div class="row col-md-6">
		...
		<?php while($data=mysqli_fetch_object($q)){ ?> 
				<tr> 
					...
					<td>
						...
						<a class="btn btn-sm btn-danger" href="user.php?act=delete&&id=<?php echo $data->id ?>">Hapus</a>
					</td> 
				</tr>
		<?php } ?>
			</tbody> 
