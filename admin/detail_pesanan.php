<?php 
	...
	$aa = mysqli_query($conn,"update pesanan SET `read`='1' where id=$_GET[id]") or die(mysqli_error());
	// if(!empty($_GET)){
		// if($_GET['act'] == 'delete'){
...
?> 
	...
	<div class="container">
		<?php
			$q = mysqli_query($conn,"select * from pesanan where id='$_GET[id]'");
			$data = mysqli_fetch_object($q);
			$ongkir = $data->ongkir;
			$kota = $data->kota;
			$dataPembayaran = mysqli_query($conn,"select * from pembayaran where id_pesanan='$data->id' and status='verified'") or die (mysqli_error());
			$totalPembayaran = 0;
			while ($d = mysqli_fetch_array($dataPembayaran)) {
				$totalPembayaran += $d['total'];
			}

			$q1 = mysqli_query($conn,"select * from detail_pesanan where pesanan_id='$data->id'");
			$totalBayar = 0;
		...
		<thead> 
			...
			<tbody> 
		 <?php 
			$q = mysqli_query($conn,"select * from detail_pesanan where pesanan_id='$_GET[id]'");
			$total = 0;
