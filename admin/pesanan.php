<?php 
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			$q = mysqli_query($conn,"delete from pesanan WHERE id='$_GET[id]'");
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($conn,"update pesanan SET nama='$nama',tanggal_pesan='$tanggal_pesan',tanggal_digunakan='$tanggal_digunakan',user_id='$user_id',alamat='$alamat',telephone='$telephone' where id=$_GET[id]") or die(mysqli_error());
			if($q){ alert("Success"); redir("pesanan.php"); }
		}
	}
...
?> 
	...
	<div class="container">
		...
		<?php
			if(!empty($_GET)){ 
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object(mysqli_query($conn,"select * from pesanan where id='$_GET[id]'"));
				?>
					...
					<form action="pesanan.php?act=edit&&id=<?php echo $_GET['id'] ?>" method="post" enctype="multipart/form-data">
						...
						<?php
							...
							<?php
								$katpro = mysqli_query($conn,"select * from user");
		...
		<?php while($data=mysqli_fetch_object($q)){ ?> 
				<tr <?php if($data->read == 0 ){ echo 'style="background:#cce9f8 !important;"'; } ?> > 
					...
					<td>
						...
						<a class="btn btn-sm btn-danger" href="pesanan.php?act=delete&&id=<?php echo $data->id ?>">Hapus</a>
					</td> 
				</tr>
		<?php } ?>
			</tbody> 
