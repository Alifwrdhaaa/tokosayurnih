		...
		<?php
			if(!empty($_GET)){
				...
				if ($_GET['act'] == "verified") {
					extract($_GET);
					$updatePembayaran = mysqli_query($conn,"update pembayaran set status = 'verified' where id='$_GET[id]'");
					if ($updatePembayaran) {
						$qPembayaran = mysqli_query($conn,"Select * from pembayaran where id_pesanan='$id_pesanan' and status='verified'") or die (mysqli_error());
						$totalPembayaran = 0;
						...
						if ($totalBayar <= $totalPembayaran) {
							$updatePesanan = mysqli_query($conn,"update pesanan set status='lunas' where id='$id_pesanan'") or die (mysqli_error());
							if ($updatePesanan) {
