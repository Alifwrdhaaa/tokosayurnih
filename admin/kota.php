<?php 
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			$q = mysqli_query($conn,"delete from kota WHERE id='$_GET[id]'");
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			extract($_POST);
			$q = mysqli_query($conn,"insert into kota Values(NULL,'$nama','$ongkir')");
			if($q){ 
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){ 
			extract($_POST);
			$q = mysqli_query($conn,"update kota SET nama='$nama',ongkir='$ongkir' where id=$_GET[id]") or die(mysqli_error());
			if($q){
				alert("Success"); redir("kota.php");
			}
		}
...
?> 
	...
	<div class="container">
		<?php
			...
			$j = mysqli_num_rows($q);
		?>
		<h4>Daftar Ongkir (<?php echo ($j>0)?$j:0; ?>)</h4>
		...
		<?php
			if(!empty($_GET)){
				...
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object(mysqli_query($conn,"select * from kota where id='$_GET[id]'"));
				?>
					<div class="row col-md-6">
		...
		<?php while($data=mysqli_fetch_object($q)){ ?> 
				<tr> 
					...
					<td>
						...
						<a class="btn btn-sm btn-danger" href="kota.php?act=delete&&id=<?php echo $data->id ?>">Hapus</a>
					</td> 
				</tr>
		<?php } ?>
			</tbody> 
